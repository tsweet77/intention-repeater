...
# Updated 10/16/2020 v1.0
# Requires word list from here: https://www.intentionrepeater.com/Python/wordlist.txt
...
max_words_response = 5

# How many seconds do you want to allow them to answer. Higher is better and of course slower.
# Note that if the think_depth is high, it can possibly take longer than this for each response.
seconds_for_each_answer = 20

# Filename of the text file with the word list.
wordlist_file_name = 'wlist_match1.txt'
...
sys.stdout.write('Intention Repeater Spiritual Chat v1.0 software created by Thomas Sweet.\n'
                 )
...
while random_seed == '':
    random_seed = int(input('Enter a Random Whole Number: '))
...
try:
    while True:
        ...
        while int(float(time.time()) - start_time) < seconds_for_each_answer:
            for c in range(1, num_words_response):
                for d in range(1, think_depth):
                    process_query.append(query_value)
                    randomize(0)
                    r = int(random.randint(0, num_words - 1))
                    wordval = word_list[r]
        ...
        sys.stdout.write('Response: ' + response_value + '\n')
        response_value = ''
        query = ''
